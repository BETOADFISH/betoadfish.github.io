...
import {type Catalog,type Question,type Region,selectedMarks,resolveSelection,banks} from './question-bank';
...
export async function createQuestionPdf(catalog:Catalog,ids:string[],role:Role,progress?:(done:number,total:number)=>void,preview=false):Promise<Uint8Array>{
 ...
 lines.push('Sources: '+(catalog.bank==='edexcel'?'Pearson Edexcel':catalog.bank==='aqa'?'AQA':'NSAA / Cambridge Assessment Admissions Testing')+'. Original rights retained.');
 lines.forEach((text,i)=>page.drawText(text,{x:42,y:470-i*35,size:10,font}));}
 async function source(url:string){if(!cache.has(url)){if(!/^\/question-bank\/sources\/[a-f0-9]{64}\.pdf$/.test(url))throw Error('原题地址无效。');const response=await fetch(url);if(!response.ok)throw Error('原题文件暂时无法下载，请稍后重试。');cache.set(url,await PDFDocument.load(await response.arrayBuffer(),{ignoreEncryption:true}));}return cache.get(url)!;}
 ...
 if(role==='ms'&&catalog.bank==='esat'){
  let answerPage:ReturnType<typeof pdf.addPage>|undefined,y=0;
  for(let i=0;i<selected.length;i++){
   const q=selected[i],p=papers.get(q.paper_id)!;const src=await source(p.ms);
   for(const region of q.ms){const original=src.getPage(region.page-1),rect=original.getCropBox(),b=region.box||[0,0,1,1];const left=rect.x+b[0]*rect.width,right=rect.x+b[2]*rect.width,bottom=rect.y+(1-b[3])*rect.height,top=rect.y+(1-b[1])*rect.height;const width=right-left,height=top-bottom;
    if(!answerPage||y-height<48){answerPage=pdf.addPage([Math.max(595.28,width+330),841.89]);answerPage.drawText('BIOLOGY — ANSWER KEY'.replace('—','-'),{x:36,y:799,size:18,font:bold,color});answerPage.drawText('Original answer rows | NSAA Biology',{x:36,y:777,size:9,font});y=740;}
 ...
 if(role==='ms'&&!preview){for(const pid of new Set(selected.map(q=>q.paper_id))){const p=papers.get(pid)!;if(p.general_ms?.length)await append(p.ms,p.general_ms,'Marking guidance',`${p.title} | ${p.code}`);}}
 ...
 await append(p[role],regions,`Question group ${i+1}`,`${p.title} | ${p.code} | Original ${q.label} | ${q.marks} marks`);progress?.(i+1,selected.length);}
 return pdf.save();
}
