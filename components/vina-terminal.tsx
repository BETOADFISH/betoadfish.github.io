...
export function VinaTerminal(){
 ...
 return <section ref={surface} className="vina-workbench" data-running={playing} data-ligand={ligand} aria-labelledby={`vina-workbench-title-${uid}`}>
  <header className="vina-window-bar"><div className="vina-window-title"><TerminalSquare size={17}/><span id={`vina-workbench-title-${uid}`}>HuBisCO <i>/</i> AutoDock Vina</span></div><span className="vina-replay-badge"><i/><Copy>{t('Workflow replay','工作流回放')}</Copy></span></header>
  <div className="vina-workspace">
   <aside className="vina-sidebar"><label htmlFor={`vina-receptor-${uid}`}><Copy>{t('Receptor model','受体模型')}</Copy></label><select id={`vina-receptor-${uid}`} value={protein} onChange={e=>{setProtein(e.target.value);setFrame(0);}}>{receptors.map(r=><option key={r}>{r}</option>)}</select><p className="vina-queue-label"><Copy>{t('Ligand queue','配体队列')}</Copy><span>{finished}/3</span></p><div className="vina-queue">{ligands.map((l,i)=><button key={l} className={i===index?'active':''} aria-pressed={i===index} aria-label={(locale==='zh'?'回放 ':'Replay ')+l} onClick={()=>replay(i*duration)}><FileCode2 size={16}/><span>{l}</span>{i<finished?<Check size={15}/>:i===index?<span className="vina-queue-dot"/>:<span className="vina-queue-number">0{i+1}</span>}</button>)}</div><p className="vina-source-type">PDBQT <span>→</span> Vina</p></aside>
