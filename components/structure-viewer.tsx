...
export function StructureViewer({ compact = false }: {
    ...
    const residue = project.residues.find((r) => r.id === selected) ?? {
        ...
        description: 'Explore the deposited wild-type dimer. Select a residue to highlight its local environment and inspect the hypothesis it informed.',
    };
    useEffect(() => {
        if (activated || !host.current)
            return;
        ...
        async function init() {
            try {
                ...
                stage = new NGL.Stage(host.current!, {
                    backgroundColor: document.documentElement.dataset.theme === 'dark' ? '#17221b' : (compact ? '#f8fbf7' : '#fafcf9'),
                    quality: 'medium',
    ...
    return (<div className={compact ? 'molecule compact-molecule' : 'molecule'}>
      {!compact && (<div className="viewer-top">
          <div>
            <span className="eyebrow"><Copy>{"Structure explorer"}</Copy></span>
            <h3><Copy>{"Inspect the molecular hypothesis."}</Copy></h3>
          </div>
          <Tabs value={structure} onValueChange={(v) => setStructure(String(v))}>
            <TabsList aria-label={tr("Select experimental structure")}>
              <TabsTrigger value="9RUB"><Copy>{"9RUB \u00B7 Bound"}</Copy></TabsTrigger>
              <TabsTrigger value="5RUB"><Copy>{"5RUB \u00B7 Apo"}</Copy></TabsTrigger>
            </TabsList>
          </Tabs>
        </div>)}
          ...
          {state !== 'ready' && (<div className="viewer-fallback">
              ...
              <p role="status">
                ...
                : activated
                    ...
                    : 'Static reference · 3D loads when in view'}</Copy>
              </p>
              {state === 'error' && (<Button variant="outline" onClick={() => setRetry((x) => x + 1)}><Copy>{"Retry 3D"}</Copy></Button>)}
            </div>)}
          <span className="structure-id"><Copy>{"PDB"}</Copy>{' '}<Copy>{structure}</Copy> <span><Copy>{"\u00B7 Wild-type reference"}</Copy></span>
          </span>
          <div className="viewer-controls">
            <Button variant="outline" aria-label={tr("Reset structure view")} disabled={state !== 'ready'} onClick={reset}>
              <RotateCcw size={17}/>
            </Button>
            <Button variant="outline" aria-label={tr("Zoom in")} disabled={state !== 'ready'} onClick={() => stageRef.current?.viewerControls.zoom(0.18)}>
              <ZoomIn size={17}/>
            </Button>
            <Button variant="outline" aria-label={tr("Zoom out")} disabled={state !== 'ready'} onClick={() => stageRef.current?.viewerControls.zoom(-0.22)}>
              <ZoomOut size={17}/>
            </Button>
            <Button variant="outline" aria-label={tr(spinning ? 'Pause rotation' : 'Start slow rotation')} aria-pressed={spinning} disabled={state !== 'ready'} onClick={() => {
            stageRef.current?.setSpin(!spinning);
            setSpinning(!spinning);
        }}>
              {spinning ? <Pause size={17}/> : <Play size={17}/>}
            </Button>
            {compact && (<SiteLink href="/projects/hubisco/#structure" className="viewer-open" aria-label={tr("Open full structure explorer")}>
                <Maximize2 size={17}/>
              </SiteLink>)}
          </div>
          <span className="drag-hint"><Copy>{"Drag to rotate \u00B7 Buttons to zoom"}</Copy></span>
        </div>
        {!compact && (<aside className="residue-panel">
            <p className="eyebrow"><Copy>{"Explore chain A"}</Copy></p>
            <div className="residue-buttons">
              {project.residues.map((r) => (<Button key={r.id} variant="outline" aria-pressed={selected === r.id} disabled={state !== 'ready' ||
                    (structure === '5RUB' &&
                        (r.id === 'Mg²⁺' || r.id === 'Loops'))} onClick={() => focus(r.id)}>
                  <Copy>{r.id}</Copy>
                </Button>))}
            </div>
            <div className="residue-explanation" aria-live="polite">
              <span className="pill"><Copy>{residue.kind}</Copy></span>
              <h3><Copy>{residue.title}</Copy></h3>
              <p><Copy>{residue.description}</Copy></p>
            </div>
            <div className="viewer-legend">
              <span>
                <i className="key chain-a"/><Copy>{"Chain A"}</Copy></span>
              <span>
                <i className="key chain-b"/><Copy>{"Chain B"}</Copy></span>
              <span>
                <i className="key metal"/><Copy>{"Mg\u00B2\u207A"}</Copy></span>
            </div>
            <p className="small muted">
              <Copy>{structure === '9RUB'
                ? 'Bound ligand: native RuBP, not HuBP. Carbamylated Lys191 includes linked FMT records.'
                : 'Nonactivated reference. No bound RuBP or Mg²⁺; substantial active-site loop regions are missing from the deposited coordinates.'}</Copy>
            </p>
            <SiteLink className="text-link" href={`https://www.rcsb.org/structure/${structure}`} target="_blank" rel="noreferrer"><Copy>{"View deposited structure \u2197"}</Copy></SiteLink>
          </aside>)}
      </div>
      {!compact && (<p className="viewer-footnote"><Copy>{"Experimental reference structures support hypothesis generation. This is not a solved structure of an engineered HuBisCO mutant."}</Copy>{' '}<Copy>{' '}</Copy>
          <SiteLink href={`/structures/${structure}.pdb`} download><Copy>{"Download"}</Copy>{' '}<Copy>{structure}</Copy>{' '}<Copy>{"PDB \u2193"}</Copy></SiteLink>
        </p>)}
    </div>);
}
