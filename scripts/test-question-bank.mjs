...
for(const [bank,cat] of Object.entries(catalogs)){
 ...
 console.log(`${bank}: ${cat.questions.length} public records and dependencies valid`);
}
const a={id:'a',leaves:['a'],is_leaf:true,dependencies:[],marks:2};const b={id:'b',leaves:['b'],is_leaf:true,dependencies:[{kind:'requires_answer',question_id:'a'}],marks:1};const c={id:'c',leaves:['c'],is_leaf:true,dependencies:[{kind:'requires_answer',question_id:'b'}],marks:1};assert.deepEqual(resolveSelection(['c'],[a,b,c]).map(x=>x.id),['a','b','c']);assert.deepEqual(removeSelection(['a','b','c'],'a',[a,b,c]),[]);assert.throws(()=>resolveSelection(['missing'],[a]));assert.throws(()=>resolveSelection(['a'],[{...a,dependencies:[{kind:'requires_answer',question_id:'a'}]}]));
const parent={...a,id:'p',kind:'major',is_leaf:false};assert.deepEqual(resolveSelection(['a','p'],[{...a,parent_id:'p'},parent]).map(x=>x.id),['p']);
const {privateKey,publicKey}=await generateKeyPair('RS256');const jwk=await exportJWK(publicKey);jwk.kid='test';const keys=createLocalJWKSet({keys:[jwk]});const env={ACCESS_TEAM_DOMAIN:'test.cloudflareaccess.com',ACCESS_AUD:'audience',OWNER_EMAIL:'owner@example.test'};
