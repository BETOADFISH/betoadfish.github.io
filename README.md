...
## Public question snapshots

Public browsers read the catalog, published edits and PDFs entirely from the portfolio origin. They never need to connect to workers.dev. The Pages workflow validates and snapshots the public API before building; a failed API read aborts publication, preserving the previously deployed snapshot. Hashed snapshot paths prevent mixing versions. The editor stays private on Cloudflare.

GitHub Actions checks for public changes every 15 minutes and skips a scheduled rebuild when the snapshot is unchanged. Publishing and withdrawal take effect after the next successful deployment; scheduling and caching can delay this. For urgent updates, use Run workflow in the Publish portfolio workflow and check its result. GitHub may disable scheduled workflows after 60 days without repository activity; re-enable the schedule in Actions if needed. No extra hosting subscription is introduced. This removes the student's Cloudflare dependency but does not guarantee access to GitHub Pages on every mainland network.
