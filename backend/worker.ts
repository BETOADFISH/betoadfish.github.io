...
import {collectDownload,downloadStats} from './analytics';
...
export default {async fetch(req:Request,env:Env):Promise<Response>{
 ...
 if(path==='/events/download')return collectDownload(req,env.DB,origin);
 ...
 if(path==='/admin/api/stats'&&req.method==='GET'){const days=Number(url.searchParams.get('days')||30);if(![7,30,90].includes(days))return json({error:'请选择7、30或90天。'},400);return json(await downloadStats(env.DB,days));}
...
},async scheduled(_event:ScheduledEvent,env:Env){await env.DB.prepare("DELETE FROM download_events WHERE created_at < datetime('now','-90 days')").run();}};
