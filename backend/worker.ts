...
export default {async fetch(req:Request,env:Env):Promise<Response>{
 ...
 if(path==='/admin/api/list'&&req.method==='GET'){const bank=url.searchParams.get('bank');if(!['edexcel','aqa','esat'].includes(bank||''))return json({error:'请选择题库。'},400);const page=Math.max(0,Math.min(1000,Number(url.searchParams.get('page'))||0));const query=(url.searchParams.get('q')||'').slice(0,100);const {results}=await env.DB.prepare("SELECT id,base,draft,published,changed,revision FROM questions WHERE bank=? AND (id LIKE ? OR json_extract(base,'summary') LIKE ?) ORDER BY id LIMIT 50 OFFSET ?").bind(bank,'%'+query+'%','%'+query+'%',page*50).all<Row>();return json(results.map(r=>{const base=JSON.parse(r.base);return{id:r.id,label:base.label,title:base.paper_title,summary:base.summary,revision:r.revision,status:r.draft?'draft':r.changed&&!r.published?'unpublished':'published'};}));}
